...
object CurriesPAF extends App {
  ...
  // Methods get lifted to function expressions
  // functions != methods (JVM limitation)

  def inc(x: Int): Int = x + 1

  List(1, 2, 3).map(inc)
  // compiler will basically translate ^ into this v
  List(1, 2, 3).map(x => inc(x)) // ETA-expansion

  // Partial function applications
  // syntax allowed in scala 2 and early scala 3, now unnecessary and soon deprecated
